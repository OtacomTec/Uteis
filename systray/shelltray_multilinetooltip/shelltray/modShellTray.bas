Attribute VB_Name = "modShellTray"
Option Explicit
...
'======== NOTE ========
'Conditional compilation is being used in this demo because
'there are several different versions of the NOTIFYICONDATA
'structure. If your version of Internet Explorer is lower then
'5.01, you need to modify the WIN32_IE constant below.
'======================
...
Public Sub ShellIconInitialize()

'======== NOTE ========
'Conditional compilation is being used in this routine because
'there are several different versions of the NOTIFYICONDATA
'structure. If your version of Internet Explorer is lower then
'5.01, you need to modify the WIN32_IE constant in the General
...
'Subclass the form
hOldProc = GetWindowLong(frmShellTray.hWnd, GWL_WNDPROC)
SetWindowLong frmShellTray.hWnd, GWL_WNDPROC, AddressOf ShellIconCallback

'Add our icon to the tray
ni.cbSize = Len(ni)
ni.hWnd = frmShellTray.hWnd
...
ni.hIcon = frmShellTray.imgIcon.Picture.Handle
ni.szTip = "Visual Basic Thunder Demo" & Chr$(0)
#If WIN32_IE >= &H500 Then 'IE 5.0 or higher - use balloon tip members
ni.szTip = "Visual Basic Thunder Demo" & Chr$(0)
ni.szInfo = "You can use balloon tips because you have IE5 or higher." & Chr$(0)
ni.uTimeout = 10000 '10000 (10 seconds) is the minimum value anyway
ni.szInfoTitle = "Shell Icon Demo"
ni.dwInfoFlags = NIIF_INFO
#End If

If Shell_NotifyIcon(NIM_ADD, ni) = 0 Then
    'Failure!
    MsgBox "Failed to initialize shell icon.", vbExclamation
End If

End Sub

Public Function ShellIconCallback(ByVal hWnd As Long, ByVal wMsg As Long, _
...
If wMsg = VBT_SHELLICON Then
    ...
    If (lParam = WM_RBUTTONUP) Or (lParam = WM_CONTEXTMENU) Then
        ...
        frmShellTray.PopupMenu frmShellTray.mnuPopup
    End If
...
If Shell_NotifyIcon(NIM_DELETE, ni) = 0 Then
    'Failure!
    MsgBox "Failed to remove shell icon.", vbExclamation
End If

'Unsubclass the shelltray form
SetWindowLong frmShellTray.hWnd, GWL_WNDPROC, hOldProc
hOldProc = 0
