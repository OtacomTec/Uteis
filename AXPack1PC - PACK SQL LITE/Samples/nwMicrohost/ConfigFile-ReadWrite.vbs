...
Sub OnInput(s)
    ...
    Host.WriteLine "Reading file (text format): " & s
    Set cfg = cf.Read(s)
    ListCfg cfg, "  "
    Host.WriteLine "Writing file (binary format W): " & Host.RuntimeParameter("ScriptPath") & "bin.cfg"
    Set sf = Host.CreateObject("newObjects.utilctls.SFMain")
    Set f = sf.CreateFile(Host.RuntimeParameter("ScriptPath") & "bin.cfg")
    cf.PreserveStringsWide = True
    cf.PreserveUnsignedInt = True
    cf.WriteToBinaryStream f, cfg
